...
모든 프로젝트(16개)는 `src/data/projects.ts` 한 파일에서 관리합니다. 홈, 프로젝트 목록, 프로젝트 상세 페이지가 같은 데이터를 참조해 정보 불일치를 차단합니다.
...
export interface Project {
  ...
  result?: string;     // 성과 칩 (예: "1등", "상위 5.2%")
  period: string;
  org: string;
  role?: string;
  summary: string;
  hero?: string;       // 일러스트 SVG 또는 실제 데모 캡처 PNG
  color?: string;      // 카드·상세 강조색
  github?: string;
  external?: { label: string; url: string }[];
  stack: string[];
  body: { heading: string; text: string }[];
  metrics?: { label: string; value: string }[];                 // 저장소 리포트 실측값
  architecture?: { image: string; alt: string; note?: string }; // 아키텍처 다이어그램
  featured?: boolean;
